...
(function () {
  ...
  // ===== Кэш групп (для подсчёта людей в комнате) =====
  ...
  async function loadGroups(){
    ...
    try{
      ...
      groupsCache.byId = new Map(arr.map(g => [Number(g.id), { id:Number(g.id), personsCount:Number(g.personsCount||0) }]));
    } finally { groupsCache.loaded = true; }
  }

  // ===== Кэш метаданных аудиторий (для фильтров) =====
  const roomsMeta = { byId: new Map(), byName: new Map(), loaded:false };
  async function loadRoomsMeta(){
    if (roomsMeta.loaded) return;
    try{
      const r = await fetch("/api/classrooms?size=1000");
      const data = r.ok ? await r.json() : [];
      const arr = Array.isArray(data) ? data : (Array.isArray(data.content) ? data.content : []);
      for (const c of arr){
    ...
    } finally { roomsMeta.loaded = true; }
  }

  ready(async () => {
    window.SchedulePanel?.init?.();
    initFloorSwitch();
    await initSlots();     // слоты только из БД
    initDates();
    initRooms();

    // данные, нужные для подсветки/фильтров
    await Promise.all([loadGroups(), loadRoomsMeta()]);

    initFilterMenu();      // после загрузки meta, чтобы фильтр работал с id
  ...
  function initFilterMenu(){
    ...
    apply?.addEventListener("click", () => {
      const facId  = facultySel.value.trim(); // "" — нейтрально
      const specId = specSel.value.trim();    // "" — нейтрально
      const rooms  = $all(".room");

      rooms.forEach(r => {
        if (r.classList.contains("foyer")) return; // фойе не фильтруем
        ...
        // когда нет метаданных — считаем "не совпадает", если фильтр выбран
